...
/* dpmg2.c */
/* a traditional Bayes mixture model */
...
/*****************************************/
void dpmg(int *dimint, double *dimdbl, double *datain, double *priorin, double *meanin, int *indout, double *phiout, 
	  double *muout, double *sigmaout, /*int *Mout,*/ char **seedstring)
{
  ...
  double kappa=dimdbl[2], invkappa=1.0/kappa;
  ...
  double *etaSIGchol, *etaTEMP;
  phi=fvector(n-1); p=fvector(n); etaMU=fmatrix(n,k); etaSIGMA=fmatrix(n,kk); 
  etaSIGchol=fvector(kk); etaTEMP=fvector(kk); etaINV=fmatrix(n,kk);
  ...
  tmpf=1.0*(rho+k+1);  // Final mpdpm program value.
  for(l=0; l<L; ++l)
    {
      for(j=0; j<kk; ++j)
	{
  ...
  for(j=0; j<n; ++j)
    {
      etaRDET[j]=iwish(k, kk, rho, Sinvchol[0], z, wk, etaINV[j], etaSIGMA[j]); /*etaSIGMAj ITS RDET and INVERSE*/
      for(row=0; row<kk; ++row) etaTEMP[row]=etaSIGMA[j][row]*invkappa;
      Grdet=chol(k, kk, etaTEMP, etaSIGchol);
      mvn(k, meanin, etaSIGchol, z, etaMU[j]); /*MEAN*/
    }
  ...
  for(loop=0; loop<Burn+Thin*B; ++loop) /*BEGIN GIBBS*/
      ...
      for(j=0; j<n; ++j)
	{
	  if(sized[j]==0)
		...
		for(col=0; col<=row; ++col)
		  {
		    SS[l]+=(xbar[row]-meanin[row])*(xbar[col]-meanin[col]);
		    ++l;
		  }
	      tmpf=(1.0*size[j]*kappa)/(1.0*size[j]+kappa);
	      for(i=0; i<kk; ++i) 
		SSS[0][i]+=SS[i]*tmpf;

	      SSSRDET[0]=chol(k,kk,SSS[0],TEMPKK);
	      ...
	      tmpf=1.0/(size[j]+kappa);
	      for(row=0; row<k; ++row) mu[row]=(1.0*kappa*meanin[row]+1.0*size[j]*xbar[row])*tmpf;
	      for(row=0; row<kk; ++row) V[row]=etaSIGMA[j][row]*tmpf;
	      chol(k, kk, V, Vchol);
